...
static int fpga_oled_open(struct inode * inode, struct file * file)
{
	if (oled_usage == -1)
		return -EBUSY;
...
static int fpga_oled_release(struct inode * inode, struct file * file)
{
...
/* not use
static ssize_t fpga_oled_read(struct file * file, char * buf, size_t length, loff_t * ofs)
{
	printk("fpga_oled_read, \n");
	
	return 0;
}
*/

static ssize_t fpga_oled_write(struct file * file, const char * buf, size_t length, loff_t * ofs)
{
...
static DEFINE_MUTEX(fpga_oled_mutex);
static long fpga_oled_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
	int ret;
...
static struct file_operations fpga_oled_fops = {
	.owner = THIS_MODULE,
	.open = fpga_oled_open,
	.release = fpga_oled_release,
//	.read = fpga_oled_read,
	.write = fpga_oled_write,
	.unlocked_ioctl = fpga_oled_ioctl,
};

static struct miscdevice fpga_oled_driver = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "fpga_oled",
	.fops = &fpga_oled_fops,
};

static int fpga_oled_init(void){
	printk("fpga_oled_init, \n");
	
	return misc_register(&fpga_oled_driver);
}

static void fpga_oled_exit(void){
	printk("fpga_oled_exit, \n");

	misc_deregister(&fpga_oled_driver);
	
}

module_init(fpga_oled_init);
module_exit(fpga_oled_exit);

MODULE_AUTHOR("Hanback");
MODULE_DESCRIPTION("fpga_oled driver test");
